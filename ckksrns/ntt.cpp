...
Integer ntt::findCloseQ(int n, Integer x)
{
    ...
    for (; i < max; ++i)
    {
        ...
        for (int j = 0; j < two; ++j)
        {
            ...
            if ( gcdT(mod, in) != U) continue;
            Integer y = find2NthRoot(mod, n, false);
            if (!!y) return mod;
        }
    }
    nevers("Cannot find close Q for " + std::to_string(x));
}

std::vector<Integer> ntt::findCoprimes(int n, Integer q0, int howmany)
{
    ...
    for (; i < max; ++i)
    {
        ...
        for (int j = 0; j < two; ++j)
        {
            ...
            {
                ret.push_back(mod);
                if ( (int)ret.size() >= howmany ) return ret;
            }
        }
    }

    nevers("Cannot find close Q for " + std::to_string(q0));
}
    ...
    for (int m = 1; m < n; m *= 2)
    {
        ...
        for (int i = 0; i < m; i++)
        {
            ...
            for (int j = j1; j <= j2; ++j)
            {
                ...
                //a.v[j] = U + V;
                //a.v[j + t] = U - V;
                auto V = modmul(a.v[j + t], S, mod);
                a.v[j] = modadd(U, V, mod);
                a.v[j + t] = modsub(U, V, mod);
            }
        }
    }
    ...
    for (int m = n; m > 1; m /= 2)
    {
        ...
        for ( int i = 0; i < h; ++i )
        {
            ...
            for ( int j = j1; j <= j2; j++ )
            {
                ...
                a.v[j] = modadd(U, V, mod);
                //a.v[j + t] = (U - V) * S;
                a.v[j + t] = modmul( modsub(U, V, mod), S, mod);
            }
            j1 = j1 + 2 * t;
        }
        t = 2 * t;
...
Integer ntt::find2NthRoot(Integer mod, int n, bool throws)
{
    ...
    if (m > Integer(1))
    {
        ...
        for (; i < m; ++i)
        {
            auto x = modpow(i, Integer(n), m); ///i.powmod(n);
            if (x == mu)
            {
                auto om = modmul(i, i, m); ///i * i; // .mulmod(i, m);
                if (om == u) continue;
...
bool ntt::chk1NthRoot(Integer mod, int n, Integer om)
{
    ...
    if (modpow(om, Integer(n), mod) != u) return false;

    if (n % 2 == 0)
    {
        if (modpow(om, Integer(n / 2), mod) == (mod - 1)) return true;
    }

    // check that all powers <n !=1
    for (int j = 1; j < n; ++j)
    {
        if (modpow(om, Integer(j), mod) == u) return false;
    }
...
bool ntt::chk2NthRoot(Integer mod, int n, Integer psi)
{
    ...
    auto x = modpow(psi, Integer(n), mod);
    if (x == mu)
    {
        auto om = modmul(psi, psi, mod);
...
void ntt::Context::build_psi_powers()
{
    ...
    for (int i = 0; i < n; i++)
        psi_powers.push_back(modpow(npsi, Integer(i), mod));
    ///npsi.powmod(i));

    auto psi1 = modinv(npsi, mod); ///npsi.invmod();
    for (int i = 0; i < n; i++)
        ps1_powers.push_back(modpow(psi1, Integer(i), mod));
    ///psi1.powmod(i));
}
...
Integer ntt::Context::find_psi(Integer mod, int n)
{
    ...
    if (!chk2NthRoot(mod, n, apsi)) nevers("bad psi " + std::to_string((apsi)) );
    return apsi;
}
