...
Integer factor_ferma(Integer n)
{
    if (n < 4) return Integer { 1 };
...
Integer factor_pollard_pm1(Integer n)
{
    if (n < 4) return Integer { 1 };
...
Integer factor_pollard_rho(Integer n)
{
    if (n < 4) return Integer { 1 };
    if (::isZero(n % 2)) return Integer { 2 };
    if (isPrime(n)) return Integer { 1 };
...
std::vector<Integer> factors(Integer n, int alg)
{
    auto ff = alg == 0 ? factor_root
              : alg == 1 ? factor_ferma
              : alg == 2 ? factor_pollard_pm1
              : alg == 3 ? factor_pollard_rho
              : factor_root;
...
std::vector<Integer> factors(Integer n)
{
